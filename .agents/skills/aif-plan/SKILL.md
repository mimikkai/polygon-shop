...
- If `--parallel` is set while `no_git_switching = true`, reject it with a short explanation: the `full-no-git-switching` mode cannot use parallel worktrees because parallel mode requires branch creation
...
- `full-no-git-switching` keyword → **parse first** (longest match): set `mode=full` and `no_git_switching=true`
...
**Internal state when `full-no-git-switching` is selected:**

- `mode = full`
- `no_git_switching = true`

This state affects Step 1.2, Step 1.4, and `--parallel` handling.
...
2. **`no_git_switching = true`** → `plan_file_stem = <description slug>`. No `branch_name` is created. This mode never creates or switches branches, regardless of git config.
3. **`git.enabled = true` AND `git.create_branches = true`** → generate a description slug, then `branch_name = <git.branch_prefix><slug>` (default prefix: `feature/`). Set `plan_file_stem = branch_name` with every `/` replaced by `-` (for example `feature-user-authentication`).
4. **Otherwise** (`git.enabled = false` OR `git.create_branches = false`) → `plan_file_stem = <description slug>`. No `branch_name` is created.

Slug rules (cases 2, 3, and 4):
...
**Invariant:** branch-based consumer skills compute their lookup stem as `current-branch-with-slashes-replaced`. Cases 1 and 3 above already match that. Cases 2 and 4 never have a branch, so consumers fall back to the lone full-mode plan in `<configured plans dir>/` (see `aif-implement` Step 0.2). Producing a `plan_file_stem` outside these rules breaks discovery.
...
**If `no_git_switching = true`:**

- Skip all branch/worktree creation — this is the core guarantee of `full-no-git-switching`.
- Do **NOT** run `git checkout`, `git pull`, `git checkout -b`, or `git worktree add`.
- Continue with the generated full plan file path under `paths.plans/<slug>.md`.
- `--parallel` was already rejected at Step 0.2.
...
8. **Plan file location** – Fast mode: `paths.plan`. Full mode: `paths.plans/<plan_file_stem>.md` by default (`plan_file_stem` = handoff branch / description slug under no-git-switching / git branch / description slug per Step 1.2.a), or `paths.plans/<NNNN>_<plan_file_stem>.md` when `workflow.plan_id_format = sequential` (see Step 1.2.c for the numbering rule and Handoff override). `timestamp` and `uuid` are reserved values and currently fall back to `slug`.
...
- The canonical `plan_file_stem` comes from Step 1.2.a: Handoff branch name (slashes replaced) → description slug (when `no_git_switching=true`) → git branch name (slashes replaced) → description slug, in that priority order
