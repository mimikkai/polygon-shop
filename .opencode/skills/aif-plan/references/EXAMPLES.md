...
### Full-no-git-switching mode

```text
/aif-plan full-no-git-switching Add OAuth login
-> mode=full, no_git_switching=true, description="Add OAuth login"
```

### Full-no-git-switching mode with --parallel (rejected)

```text
/aif-plan full-no-git-switching --parallel Add OAuth login
-> REJECTED: the full-no-git-switching mode cannot use --parallel because parallel mode requires branch creation
```
...
### Scenario 6: Full-no-git-switching mode (no branch creation, even with git enabled)

```text
/aif-plan full-no-git-switching Add OAuth login

Assumes config: git.enabled=true, git.create_branches=true

-> mode=full, no_git_switching=true
-> Quick reconnaissance
-> Plan slug: oauth-login
-> NO branch created (no_git_switching overrides git.create_branches)
-> plan_file_stem = oauth-login (description slug, NOT branch-based)
-> If ROADMAP.md exists: asks about milestone linkage, user picks one (or skips)
-> Asks about tests (Yes), logging (Verbose), docs (Yes)
-> Step 1.4 is skipped entirely — no git checkout, no git pull, no git checkout -b
-> Explores codebase deeply
-> Creates 8 tasks with commit checkpoints
-> Saves plan to paths.plans/oauth-login.md
-> STOP - user runs /aif-implement when ready
```

### Scenario 7: Full-no-git-switching with sequential numbering
...
-> mode=full, no_git_switching=true
-> workflow.plan_id_format = sequential
-> Plan slug: oauth-login
-> NO branch created
-> Next number: 0001 + 1 = 0002
-> Saves plan to paths.plans/0002_oauth-login.md
-> STOP - user runs /aif-implement when ready
```
